...
def get_data_filename(filename="json_data") -> str:
    return os.path.join(ASSETS_DIR, filename + ".json.gz")


def write_asset_json(data, filename="json_data", update=False) -> None:
    json_asset: str = get_data_filename(filename)
    try:
...
def find_matches_db(collection, puuid):
    matches_asset = []
    match_asset = find_collection_db(collection, "info.participants.puuid", puuid)
    if match_asset:
        matches_asset.extend(match_asset)
...
def write_collection_db(data, collection, update=False) -> None:
    try:
...
def insert_collection_db(data, collection) -> None:
    try:
        collection.insert_many(data, ordered=False)
    except Exception as e:
        logging.error(e)
